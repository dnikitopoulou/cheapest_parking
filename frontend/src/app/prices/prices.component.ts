...
export class PricesComponent  implements OnInit {
  ...
  onProdSelect(id, event) {
    ...
    if (this.cntprod === 0) {
      ...
      if (!this.products.includes(',' + event.target.value + ',') && event.target.value !== this.prodfirst &&
        event.target.value !== this.prodlast) {
  ...
  onShopSelect(id, event) {
    ...
    if (this.cntshop === 0) {
      ...
      if (!this.shops.includes(',' + event.target.value + ',') && event.target.value !== this.shopfirst &&
        event.target.value !== this.shoplast) {
  ...
  onSortSelect(id, event) {
    ...
    this.data.getPrices(this.getsort, this.dist, this.lng, this.lat, this.datefrom, this.dateto, this.products, this.shops, this.tags )
      .subscribe(data => {
        this.pric = data;
        console.log(this.pric);
      }
